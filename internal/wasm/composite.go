...
const (
	...
	// companion ValueType / RefInfo on FieldType describes the storage.
...
// Either Packed names a packed-integer storage type, or (ValueType, RefInfo)
// describe a value-type storage. Exactly one path is meaningful per instance.
type FieldType struct {
	// Packed, when not PackedTypeNone, takes precedence over ValueType /
	// RefInfo. Reads of a packed field require struct.get_s / struct.get_u
	// (array.get_s / array.get_u) to choose sign-extension.
	Packed PackedType
	// ValueType is the field's value-type byte when Packed == PackedTypeNone.
	ValueType ValueType
	// RefInfo, when non-nil, carries rich reference info for a field whose
	// ValueType byte encodes a sentinel ref (non-nullable or concrete).
	// nil when ValueType is a numeric type, vector, or a nullable abstract
	// reference shorthand.
