...
func (m *Module) validateFunctionWithMaxStackValues(
	...
	for pc := uint64(0); pc < uint64(len(body)); pc++ {
		...
		if OpcodeI32Load <= op && op <= OpcodeI64Store32 {
			...
			valueTypeStack.resetAtStackLimit()
			for i, exp := range bl.blockType.Results {
				var ref *ValueTypeRef
				if i < len(bl.blockType.ResultRefInfos) {
					ref = bl.blockType.ResultRefInfos[i]
				}
				valueTypeStack.pushRef(exp, ref)
			}
			...
			case OpcodeGCStructGet, OpcodeGCStructGetS, OpcodeGCStructGetU:
				...
				// Push field's unpacked-stack type.
				if packed {
					valueTypeStack.push(ValueTypeI32)
				} else {
			...
			case OpcodeGCArrayFill:
				...
				// pop count, value, index, array ref
				if err := valueTypeStack.popAndVerifyType(ValueTypeI32); err != nil {
			...
			case OpcodeGCArrayNewData, OpcodeGCArrayNewElem:
				...
				_ = segIdx // segment index validity is checked at instantiation
				// Pop count (i32), src offset (i32). Push array ref.
				if err := valueTypeStack.popAndVerifyType(ValueTypeI32); err != nil {
			...
			case OpcodeGCBrOnCast, OpcodeGCBrOnCastFail:
				...
				// src and dst heap-type immediates per the spec encoding.
				pc++
				if int(pc) >= len(body) {
...
// fields take their nullable shorthand byte.
//
// Returns an error for ref-typed fields that the current minimal heap
// layer does not yet support storing.
