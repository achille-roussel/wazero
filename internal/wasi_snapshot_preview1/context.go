...
func (ctx *Context) Register(f sys.File, fsRightsBase, fsRightsInheriting Rights) Fd {
...
func (ctx *Context) Lookup(fd Fd) sys.File {
...
func (ctx *Context) FS() sys.FS { return contextFS{ctx} }
...
func (fsys contextFS) OpenFile(path string, flags int, perm fs.FileMode) (sys.File, error) {
...
func (f *contextFile) OpenFile(path string, flags int, perm fs.FileMode) (sys.File, error) {
	if f.fd == None {
