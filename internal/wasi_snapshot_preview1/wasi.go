...
func makeError(errno Errno) error {
	...
	case EBADF:
		return fs.ErrClosed
	case ENOSYS:
		return sys.ErrNotImplemented
	case EROFS:
		return sys.ErrReadOnly
	default:
		return errno
	}
}
...
func makePathOpenFlags(flags int, perm fs.FileMode) (dirflags Lookupflags, oflags Oflags, fsRightsBase, fsRightsInheriting Rights, fdflags Fdflags) {
	switch {
	case (flags & sys.O_RDWR) != 0:
		fsRightsBase = RW
	case (flags & sys.O_WRONLY) != 0:
		fsRightsBase = W
	default:
		fsRightsBase = R
	}
	...
	if (flags & sys.O_CREATE) != 0 {
		oflags |= O_CREAT
	}
	if (flags & sys.O_EXCL) != 0 {
		oflags |= O_EXCL
	}
	if (flags & sys.O_SYNC) != 0 {
		fdflags |= FD_SYNC
	}
