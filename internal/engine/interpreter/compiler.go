...
operatorSwitch:
	...
	case wasm.OpcodeCallRef, wasm.OpcodeReturnCallRef:
		...
		if !c.unreachableState.on {
			...
			// Push results (return_call_ref skips this in unreachable
			// mode handled by the validator path).
	...
	case wasm.OpcodeBrOnNull, wasm.OpcodeBrOnNonNull:
		...
		if op == wasm.OpcodeBrOnNull {
			...
			// On fall-through the ref is back on the stack.
			c.stackPush(unsignedTypeI64)
		} else {
			c.emit(newOperationBrOnNonNull(target, continuationLabel, drop))
			// On fall-through the ref is consumed; on the branch path it
			// is passed to the label. We do NOT push back here because
			// the local "fall-through" execution path has no ref.
		}
		c.emit(newOperationLabel(continuationLabel))
