...
const (
	...
	// operationKindAnyConvertExtern is the Kind for any.convert_extern: pop an externref,
	// push it as an anyref. The runtime representation is identical so this is a no-op.
	operationKindAnyConvertExtern

	// operationKindExternConvertAny is the Kind for extern.convert_any: pop an anyref,
...
// newOperationRefI31 constructs the operation for ref.i31: pop i32, allocate
// an *wasm.I31Ref, push its keepalive-registered uintptr.
func newOperationRefI31() unionOperation {
	return unionOperation{Kind: operationKindRefI31}
}

// newOperationI31GetS constructs the operation for i31.get_s: pop a *wasm.I31Ref
// (trapping if null), push its sign-extended i32.
func newOperationI31GetS() unionOperation {
	return unionOperation{Kind: operationKindI31GetS}
}

// newOperationI31GetU constructs the operation for i31.get_u: pop a *wasm.I31Ref
// (trapping if null), push its zero-extended i32.
func newOperationI31GetU() unionOperation {
	return unionOperation{Kind: operationKindI31GetU}
}

// newOperationRefEq constructs the operation for ref.eq: pop two refs, push 1
// if they are reference-equal (or both null) else 0. Special-cases *wasm.I31Ref
// pairs to compare by numeric value per spec.
...
// pop a reference; if it is null, trap; otherwise push it back unchanged.
...
// Runtime representation is identical to externref, so this is a no-op pass-through.
