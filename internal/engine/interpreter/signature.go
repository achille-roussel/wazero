...
func (c *compiler) wasmOpcodeSignature(op wasm.Opcode, index uint32) (*signature, error) {
	...
	case wasm.OpcodeGCPrefix:
		...
		// function body at the matching position by the compiler).
		switch index {
		case wasm.OpcodeGCRefI31:
			// i32 -> i31ref (one uint64 slot).
		...
		case wasm.OpcodeGCStructNew, wasm.OpcodeGCStructNewDefault,
			...
			// Stack manipulation handled dynamically by the compiler
			// (depends on the struct's field schema). Mirror the Throw /
			// TryTable pattern.
			return signature_None_None, nil
		default:
...
func wasmValueTypeTounsignedType(vt wasm.ValueType) unsignedType {
	...
	case wasm.ValueTypeI64,
		...
		// wasm-gc nullable abstract shorthand bytes — all same uint64 representation.
