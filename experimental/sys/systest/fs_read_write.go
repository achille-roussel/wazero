...
type MakeReadWriteFS func() (sys.FS, CloseFS, error)
...
func TestReadWriteFS(t *testing.T, newFS MakeReadWriteFS) {
	...
	t.Run("sub", func(t *testing.T) {
		testReadWriteFS(t, func() (sys.FS, CloseFS, error) {
...
func testReadWriteFS(t *testing.T, newFS MakeReadWriteFS) {
	tests := []struct {
		...
		{
			scenario: "files are created when opened with sys.O_CREATE",
			function: testReadWriteFSCreateFileWithOpen,
		},

		{
			scenario: "files cannot be recreated when opened with sys.O_EXCL",
			function: testReadWriteFSCannotCreateExistingFile,
		},

		{
			scenario: "files can be truncated when opened with sys.O_TRUNC",
	...
	file := assertOpenFile(t, fsys, "foo", sys.O_CREATE, 0644)
	defer file.Close()
...
func testReadWriteFSCreateFileWithOpen(t *testing.T, newFS MakeReadWriteFS) {
	...
	f0 := assertOpenFile(t, fsys, "foo", sys.O_CREATE, 0644)
	defer assertClose(t, f0)

	// OK because O_EXCL is not set
	f1 := assertOpenFile(t, fsys, "foo", sys.O_CREATE, 0644)
...
func testReadWriteFSCannotCreateExistingFile(t *testing.T, newFS MakeReadWriteFS) {
	...
	_, err := fsys.OpenFile("foo", sys.O_CREATE|sys.O_EXCL, 0644)
...
func testReadWriteFSTruncateFileWithOpen(t *testing.T, newFS MakeReadWriteFS) {
	...
	f0 := assertOpenFile(t, fsys, "foo", sys.O_RDWR|sys.O_CREATE, 0644)
	defer assertClose(t, f0)

	assertWrite(t, f0, "Hello World!")

	f1 := assertOpenFile(t, fsys, "foo", sys.O_RDWR|sys.O_TRUNC, 0)
	defer assertClose(t, f1)
...
func testReadWriteFSTruncateReadOnly(t *testing.T, newFS MakeReadWriteFS) {
	...
	f0 := assertOpenFile(t, fsys, "foo", sys.O_WRONLY|sys.O_CREATE, 0644)
	defer assertClose(t, f0)
	assertWrite(t, f0, "123")

	f1 := assertOpenFile(t, fsys, "foo", sys.O_RDONLY, 0644)
	defer assertClose(t, f1)
...
func testReadWriteFSTruncateToSameSize(t *testing.T, newFS MakeReadWriteFS) {
	...
	f := assertOpenFile(t, fsys, "foo", sys.O_RDWR|sys.O_CREATE, 0644)
